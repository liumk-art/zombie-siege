...
public class TowerObject : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (info.atkType == 1)
        {
            ...
            // 判断怪物和炮塔之间的夹角小于某个范围才能攻击怪物 并且 时间间隔大于攻击间隔时间
            if (Vector3.Angle(head.forward, monsterPos - head.position) < 5 && Time.time - nowTime >= info.offsetTime)
            {
                ...
                // 创建开火特效
                GameObject effObj = Instantiate(Resources.Load<GameObject>(info.eff), gunPoint.position, 
                    gunPoint.rotation);
                // 延迟移除特效
                Destroy(effObj, 0.2f);
                // 记录开火时间
